...
if __name__ == '__main__':
    ...
    for count_of_sim in [50, 100, 200, 400]:
        ...
        for count_of_envs in envs:
            ...
            print('count of simulations', count_of_sim)
            ...
            write_to_file('GPU_s_' + str(count_of_sim), data)
            print('GPU', result)

            for count_of_cpus in [5, 10, 25]:
                data = []
                for i in range(count_of_replications):
                    with torch.no_grad():
                        start = datetime.datetime.now()
                        ...
                        data.append((datetime.datetime.now() - start).total_seconds())
                result = np.average(data)
                write_to_file('CPU_GPU_P' + str(count_of_cpus) + '_s_' + str(count_of_sim), data)
            ...
            for i in range(count_of_replications):
                with torch.no_grad():
                    ...
                    data.append((datetime.datetime.now() - start).total_seconds())
            result = np.average(data)
            write_to_file('GPU_b_' + str(count_of_sim), data)
            print('GPU', result)
